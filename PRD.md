# PRD — Consolidate resume + tool into one repo at `fieldflo.bartosuerte.com`
...
- **Interactive resume** → served at the **root** `https://fieldflo.bartosuerte.com/`
- **Applicant Review Tool** → served at **`/app`** → `https://fieldflo.bartosuerte.com/app`
...
This folder (`fieldflo.bartosuerte.com/`) already contains:
...
- Resume `og:url` → `https://fieldflo.bartosuerte.com/`.
- Tool → resume: "View Interactive Resume" button now `href="/"` (same-tab).
- Tool `og:url` → `https://fieldflo.bartosuerte.com/app`.
...
4. Create a **public** GitHub repo (suggest name `fieldflo` or `fieldflo-bartosuerte`) and push. (No README/license/.gitignore added on github.com — keep the remote empty so the first push isn't rejected.)
...
- Point/confirm the `fieldflo` subdomain's document root, upload the **whole tree** (root files + `/app` subfolder).
...
On `https://fieldflo.bartosuerte.com/app`:
...
- [ ] `https://fieldflo.bartosuerte.com/` shows the full resume; all sections render from `content.js`.
- [ ] Resume's "Applicant Review Tool" CTA, proof button, proof image, and footer link all open `/app`.
- [ ] `https://fieldflo.bartosuerte.com/app` shows the tool; "View Interactive Resume" returns to `/`.
...
5. Decide later whether to 301 the old subdomains to fieldflo (currently kept live in parallel per Barton's choice).
